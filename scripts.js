...
// ─────────────────────────────────────────
// LINE FADE (whole-line opacity fade, no blur or transform)
// Apply to any element with data-animate="line-fade"
// Optional: data-delay="400" for stagger (ms)
// ─────────────────────────────────────────
function initLineFades() {
  const elements = document.querySelectorAll('[data-animate="line-fade"]');
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        const delay = parseInt(entry.target.dataset.delay || 0);
        setTimeout(() => {
          entry.target.style.transition = 'opacity 0.7s ease';
          entry.target.style.opacity = '1';
        }, delay);
        observer.unobserve(entry.target);
      }
